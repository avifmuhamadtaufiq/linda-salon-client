...
import { CreateBookingM, CreateBookingMVariables, MeQ }  from 'types/schema-types'
import CREATE_BOOKING from 'queries/booking/createBookingM.graphql'
import ME_QUERY3 from 'queries/user/meQ.graphql'

type State = typeof initialState

class CreateBooking extends Mutation<CreateBookingM, CreateBookingMVariables> {}
...
class ModalCreateBooking extends Component<Props, State> {
  ...
  private handleOk = (createBookingM: MutationFn<CreateBookingM, CreateBookingMVariables>) => () => {
    ...
    if ( endDate !== '' && startDate !== '' && createBookingM) {
      createBookingM({
        ...
        update: (client, result) => {
          const readQuery = client.readQuery<MeQ>({ query: ME_QUERY3 })
          const { data } = result

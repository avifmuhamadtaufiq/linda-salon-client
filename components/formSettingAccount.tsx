...
import { UploadUserPhoto, UploadUserPhotoVariables, DeleteUserPhotoVariables, DeleteUserPhoto, MeQ, UpdateUser, UpdateUserVariables } from 'types/schema-types'
...
class FormSettingAccount extends Component<Props, {}> {
  ...
  componentDidMount() {
    ...
    const dataMe = client.readQuery({ query: ME_QUERY }) as MeQ
    form.setFieldsValue({
      name: dataMe.me.name,
  ...
  render() {
    ...
    const { me } = client.readQuery({ query: ME_QUERY }) as MeQ
